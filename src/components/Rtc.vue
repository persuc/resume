...
import RtcHost from '@/components/RtcHost.vue'
import RtcJoin from '@/components/RtcJoin.vue'
...
const emit = defineEmits(['connected', 'message'])

const role: Ref<Role> = ref(Role.INITIAL)
const peerConnection = new RTCPeerConnection({
  // thanks to https://gist.github.com/sagivo/3a4b2f2c7ac6e1b5267c2f1f59ac6c6b
  iceServers: [
    ...[
...
peerConnection.addEventListener('icecandidate', event => {
  console.log('found candidate', event)
  // event.candidate
})
...
function onConnectionStateChange(event: Event) {
  if (peerConnection.connectionState === 'connected') {
    ...
    dataChannel = peerConnection.createDataChannel('todoLabel')
    dataChannel.addEventListener('message', onMessage)
    emit('connected', dataChannel)
  } else {
...
<template>
  <Stepper :step="step">
    <template #1>
      <div class="flex justify-center gap-2 my-32">
        <Button @click="role = Role.HOSTING; step++" class="w-fit">Host a Game</Button>
        <Button @click="role = Role.JOINING; step++" class="w-fit">Join a Game</Button>
      </div>
    </template>
    <template #2>
      <Suspense v-if="role === Role.HOSTING">
        <div>
          <RtcHost :peer-connection="peerConnection">
            <p>Send this RTC offer to your partner (click to copy):</p>
          </RtcHost>
      ...
      <RtcJoin v-else :peer-connection="peerConnection" :answer="true" @onanswer="a => { answerSdp = a; step++ }">
        <p class="mt-4 mb-8">Your partner should send you an RTC offer. Put it here:</p>
      </RtcJoin>
    </template>
    <template #3>
      <RtcJoin v-if="role === Role.HOSTING" :peer-connection="peerConnection" @onanswer="step++">
        <p class="mt-4 mb-8">Your partner should send you an RTC offer. Put it here:</p>
      </RtcJoin>
      <div v-else>
        <p>Send this RTC offer to your partner (click to copy):</p>
        <CopyButton :value="answerSdp" style="line-height: 1.5em;">
          <span class="p-2 bg-gray-200 break-words mr-30">{{ answerSdp }}</span>
        </CopyButton>
        <Button @click="step++">Next</Button>
      </div>
    </template>
