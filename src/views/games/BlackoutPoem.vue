...
<template>
  <div class="blackout-poem px-8 pt-8" style="max-width: 60rem; margin: 0 auto;">
    <div v-show="words.length === 0">
      <p>This is a freeform game where you are given a snippet of text by a famous author, and the goal is to create your
        own poem by "blacking out" or removing some of the words.</p>
    </div>
    ...
    <div v-show="words.length > 0 && !loading">
      <div class="border rounded py-1 px-3 mb-3">
        <div class="flex" style="align-items: baseline;">
          <p class="text-lg">{{ currentBook.title }}</p>&nbsp;<i>by {{ books[author].author }}</i>
        </div>
        <template v-for="(word, i) in words" :key="word + i">
          <span v-if="word !== LINE_BREAK_SYMBOL" v-html="word + '&nbsp;'"
            @click="() => black.has(i) ? black.delete(i) : black.add(i)" :class="{ word, black: black.has(i) }"></span>
          <template v-else>
            <br class="my-4" />
          </template>
        </template>
    ...
    <a v-else href="/" class="nohover" style="display: block; width: fit-content; position: relative; left: -32px;">
